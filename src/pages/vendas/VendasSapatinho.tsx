...
const VendasSapatinho: React.FC = () => {
  const { user } = useAuth()
  const { getProducts, getBalance, makeSale } = useData()
  const { canSellLojinha } = usePermissions()
  ...
  useEffect(() => {
    if (!canSellLojinha) {
      setMessage({ type: 'error', text: 'Você não tem permissão para realizar vendas no Sapatinho Veloz' })
      return
    }
  ...
  const loadProducts = () => {
    const sapatinhoProducts = getProducts('sapatinho')
    setProducts(sapatinhoProducts)
  }
  ...
  const handleFinalizeSale = async () => {
    ...
    try {
      const saleId = makeSale(selectedUserId, user?.id || '', cart, 'sapatinho')
  ...
  if (!canSellLojinha) {
    return (
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-red-200 p-8 text-center">
        ...
        <p className="text-red-600">Você não tem permissão para realizar vendas no Sapatinho Veloz.</p>
      </div>
    )
  }

  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-pink-200 p-8">
      <h2 className="text-2xl font-bold text-black mb-6">👠 Vendas - Sapatinho Veloz</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Produtos */}
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-black">✨ Itens do Sapatinho</h3>
        ...
        <div className="mt-8">
          ...
          <div className="mt-4 flex space-x-4">
            <Button
              ...
              className="bg-pink-500 hover:bg-pink-600 text-white px-6 py-3 disabled:opacity-50"
            >
...
export default VendasSapatinho
