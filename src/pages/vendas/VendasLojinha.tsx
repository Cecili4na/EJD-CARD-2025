...
const VendasLojinha: React.FC = () => {
  ...
  useEffect(() => {
    if (!canSellLojinha) {
      setMessage({ type: 'error', text: 'Você não tem permissão para realizar vendas na lojinha' })
      return
    }
  ...
  const loadProducts = () => {
    const lojinhaProducts = getProducts('lojinha')
    setProducts(lojinhaProducts)
  }
  ...
  const handleFinalizeSale = async () => {
    ...
    try {
      const saleId = makeSale(selectedUserId, user?.id || '', cart, 'lojinha')
  ...
  if (!canSellLojinha) {
    return (
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-red-200 p-8 text-center">
        ...
        <p className="text-red-600">Você não tem permissão para realizar vendas na lojinha.</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-yellow-200 p-8">
      <h2 className="text-2xl font-bold text-black mb-6">🏪 Vendas - Lojinha</h2>
